...
def prepare_document_for_signing
  ...
  # Optional: CC emails
  cc_emails = ["manager@company.com", "legal@company.com"]

  # Create multipart form data
  uri = URI.parse("#{BASE_URL}/turbosign/single/prepare-for-signing")
  ...
  if result['success']
    puts "✅ Document sent for signing"
    puts "Document ID: #{result['documentId']}"
    puts "Message: #{result['message']}"
    puts "\n📧 Emails are being sent to recipients asynchronously"
    puts "💡 Tip: Set up webhooks to receive notifications when signing is complete"
  else
