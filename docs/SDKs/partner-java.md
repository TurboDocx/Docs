...
PartnerOrgPreferencesResponse result = client.turboPartner()
    .getOrganizationPreferences("org-uuid-here");
...
Update one or more of an organization's TurboSign display preferences. Only the keys you set are changed; every other org setting is preserved. The response echoes the full, merged preferences.
