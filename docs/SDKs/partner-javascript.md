...
```typescript
const { data } = await TurboPartner.getOrganizationPreferences('org-uuid-here');
...
Update one or more of an organization's TurboSign display preferences. Only the keys you pass are changed; every other org setting is preserved. The response echoes the full, merged preferences.

```typescript
const { data } = await TurboPartner.updateOrganizationPreferences(
  'org-uuid-here',
  {
