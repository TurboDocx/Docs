---
title: How to Add Wrike Signature Anchors
sidebar_position: 6
...
- An e-signature automation configured (see [How to Setup E-Signature Automation](/docs/Integrations/Wrike/signature-automation))
...
:::tip
Repeat steps 3–6 for each signature variable in your template (e.g., `{SalesSignerName}`, `{SalesSignerSignature}`, `{ClientSignerName}`, etc.).
:::

:::caution Anchor Names Must Match Your E-Signature Automation
The signature anchor variable names you configure here **must exactly match** the anchor tags in your [How to Setup E-Signature Automation](/docs/Integrations/Wrike/signature-automation). For example, if you set up `{SalesSignerSignature}` as an anchor here, the anchor tag in your e-signature automation must also be `{SalesSignerSignature}`. If they don't match, TurboSign won't be able to place the signature fields. See [How to Setup E-Signature Automation](/docs/Integrations/Wrike/signature-automation) for how to configure the anchor tags on the automation side.
:::
